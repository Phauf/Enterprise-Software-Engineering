...
if (isset($_POST['submit']))
{
	...
	{
		case "name":
			$sql="Select `name`,`upload_date`,`upload_by`,`autoid` from `documents` where `name` like '%$searchString%'";
			break;
		case "uploadBy":
			$sql="Select `name`,`upload_date`,`upload_by`,`autoid` from `documents` where `upload_by` like '%$searchString%'";
			break;
		case "uploadDate":
			$sql="Select `name`,`upload_date`,`upload_by`,`autoid` from `documents` where `upload_date` like '%$searchString%'";
			break;
		case "all":
			$sql="Select `name`,`upload_date`,`upload_by`,`autoid` from `documents`";
			break;
		default:
	...
	while ($data=$result->fetch_array(MYSQLI_ASSOC))
	{
		...
		echo '<td><a href="view.php?fid='.$data['autoid'].'">View</a></td>';
		echo '</tr>';
	}
	echo '</table>';
}
